...
// WLED-MM true-32EQ extension. Legacy 16-band AudioSync stays unchanged.
...
static void interpolateLegacyGEQ32() {
  for (uint8_t i=0; i<NUM_GEQ_CHANNELS_32; i++) {
    const uint8_t a=i>>1;
    const uint8_t b=min((uint8_t)(a+1),(uint8_t)(NUM_GEQ_CHANNELS-1));
    fftResult32[i]=(i&1)
      ? (uint8_t)(((uint16_t)fftResult[a]+fftResult[b])>>1)
      : fftResult[a];
  }
}
...
static void runGEQ32HighPass(uint16_t numSamples, float *sampleBuffer);       // 25 Hz subsonic HPF
...
static void build32BandGEQ(float windowCorrection, bool noiseGateOpen, bool fftDataValid);
...
void FFTcode(void * parameter)
{
  ...
  for(;;) {
   ...
   if ((useInputFilter > 0) && (useInputFilter < 99)) {
    ...
    // Apply the 25 Hz HPF only to fresh samples. In sliding-window mode
    // samplesStart/sampleCount address the new 50%, so overlapping samples are
    // not filtered twice and oldSamples stores already-filtered data.
    runGEQ32HighPass(sampleCount, samplesStart);
    ...
    // Do not read vReal as FFT bins on skipSecondFFT cycles: on those cycles it
    // still contains time-domain samples.
    build32BandGEQ(wc, (fabsf(volumeSmth) > 0.25f), ((skipSecondFFT == false) || (isFirstRun == true)));
...
static void runGEQ32HighPass(uint16_t numSamples, float *sampleBuffer)
{
  constexpr float GEQ32_HPF_ALPHA =
    1.0f / (1.0f + (6.28318530717958647692f * 25.0f / (float)SAMPLE_RATE));
  static float prevIn=0.0f;
  static float prevOut=0.0f;
  for (uint16_t i=0; i<numSamples; i++) {
    const float x=sampleBuffer[i];
    const float y=GEQ32_HPF_ALPHA*(prevOut + x - prevIn);
    prevIn=x;
    prevOut=y;
    sampleBuffer[i]=y;
  }
  ...
  public:
    ...
    void setup() override
    {
      ...
      if (!initDone) {
        ...
        um_data->u_data[12] = fftResult32;       // true local 32-band FFT or interpolated AudioSync data
    ...
    void onUpdateBegin(bool init) override
    {
      ...
      memset(fftCalc32, 0, sizeof(fftCalc32));
